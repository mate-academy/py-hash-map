...
class Dictionary:
    def __init__(self, size: int = 8, load_factor: float = 0.66):
        self.size = size
    ...
    def __setitem__(self, key, value) -> None:
        ...
        while end_index != index:
            if (is_in_table := (self.hash_table[index] and self.hash_table[index][1] == key)) or self.hash_table[index] is None:
                if not is_in_table:
                    self.length += 1
